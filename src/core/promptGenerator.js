...
export function buildNextLessonPacket(progress, plan) {
  ...
  return {
    ...
    projectSnapshot: {
      projectId: progress.projectId,
      nativeLanguage: progress.learnerProfile.nativeLanguage,
...
export function renderNextLessonPrompt(packet) {
  ...
  return [
    ...
    `- Instruction language (main language for explanations/chat): ${packet.projectSnapshot.nativeLanguage}`,
    `- Teaching target language (what the learner is learning/practicing): ${packet.projectSnapshot.targetLanguage}`,
    `- IMPORTANT: Do NOT run the overall conversation primarily in ${packet.projectSnapshot.targetLanguage}.`,
    "",
    "## Lesson Context",
    `- Project: ${packet.projectSnapshot.projectId}`,
    `- Learner native language (main instruction language): ${packet.projectSnapshot.nativeLanguage}`,
    ...
    "- This must be an interactive chat lesson in dialogue form: teacher turn -> learner turn -> teacher feedback/correction/hint -> next task.",
    "- Run exercises one after another. Do not batch multiple exercises that require separate learner answers into one message.",
    "- After each learner answer, give a short correction or confirmation plus a hint/explanation before the next prompt.",
    "- First response: warm greeting + lesson goal + short pre-teaching overview of today's new grammar/verbs/vocabulary + first short exercise only.",
    "- Pre-teaching overview must assume the learner may know none of the new items yet (especially for A1/A2). Introduce meaning/usage gently before testing.",
    "- Do not start with a difficult production task before introducing the target items.",
    ...
    `- Main conversation language MUST be ${packet.projectSnapshot.nativeLanguage} (for explanations, instructions, corrections, hints, and task setup).`,
    `- The target language (${packet.projectSnapshot.targetLanguage}) is what you teach and practice, not the main language of the chat.`,
    `- Use ${packet.projectSnapshot.targetLanguage} only inside examples, prompts, mini-dialogues, and learner answer tasks.`,
    `- IMPORTANT anti-drift rule: when the learner answers in ${packet.projectSnapshot.targetLanguage}, continue your corrections/explanations in ${packet.projectSnapshot.nativeLanguage}.`,
    `- Learner answers being in ${packet.projectSnapshot.targetLanguage} is expected and required for practice; it does NOT change your teaching language.`,
    `- If you accidentally start explaining in ${packet.projectSnapshot.targetLanguage}, immediately switch back to ${packet.projectSnapshot.nativeLanguage}.`,
    `- A playful mixed style is allowed, but at least ~80% of your teacher explanations/instructions should stay in ${packet.projectSnapshot.nativeLanguage}.`,
